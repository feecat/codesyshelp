...
除此之外，您还可以参考 [施耐德的HMI设计指南（英文）](https://www.se.com/ww/en/download/document/VD-userguide-HMI/){target=_blank} 。
...
TIP: **圆形无限回转的设置**
General页中可以选择轴的模式是旋转（Modulo）或线性（Finite），这部分翻译容易造成误解。当轴驱动的对象为旋转工作台或传送带时，我们一般仍然选择运动模式为线性，这样不会有最短路径的问题。一般来说，只有使用凸轮或电子齿轮时会将轴的运动模式设为旋转。
